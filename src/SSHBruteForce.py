    ...
    def singleTarget(self, options):
        ...
        if bool(options.typeOfAttack):
            if options.combolistFile:
                ...
                tupleList = []
                user1 = open(options.usernamesFile,'r') #Util.fileContentsToList(options.usernamesFile)
                for line in user1.readlines():
                    newLine = line.replace('\n', '')
                    newTuple = (newLine[:line.find(':')], newLine[line.find(':') + 1:])
                    tupleList.append(newTuple)
                self.usernames = tupleList
                # print(self.usernames)
                pass1 =  open(options.passwordsFile,'r')  #Util.fileContentsToList(options.passwordsFile)
                tupleList = []
                for line in pass1.readlines():
                    newLine = line.replace('\n', '')
                    newTuple = (newLine[:line.find(':')], newLine[line.find(':') + 1:])
                    tupleList.append(newTuple)
                self.passwords = tupleList
            self.showStartInfo()
